...
function normalizarTelefono(tel) {
  ...
  // Si empieza con 54, sacar el 54
  if (digits.startsWith('54') && digits.length >= 12) {
    digits = digits.slice(2);
  }

  // Si empieza con 0 (ej: 0299...), sacar el 0
  if (digits.startsWith('0')) {
    digits = digits.slice(1);
  }

  // Si no tiene el 9 de celular y tiene 10 dígitos (código área + número), agregar 9
  // Ejemplo: 2993287480 → 92993287480
...
function validarTelefonoArgentino(tel) {
  ...
  // Debe quedar algo como 9 + código de área (2-4 dígitos) + número (6-8 dígitos) = 10 a 13 dígitos
  // Formato típico celular: 9XXXXXXXXXX (11 dígitos)
...
app.post('/api/solicitudes', async (req, res) => {
  try {
    ...
    // Guardamos el teléfono normalizado (solo dígitos con 9)
